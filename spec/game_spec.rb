...
module TicTacToe
  describe Game do
    let(:bob) { Player.new({team: 'X', name: 'bob'}) }
    let(:frank) { Player.new({team: 'O', name: 'frank'}) }
    ...
    context '#get_move' do
      ...
      it 'conversts human_move of 7 to [0, 2]' do
        game = Game.new([bob, frank])
        expect(game.get_move('7')).to eq([0, 2])
      end
    end
    context '#game_over_message' do
      it 'returns "{current player name} won!" if board show a winner' do
        game = Game.new([bob, frank])
        game.stub(:current_player) { bob }
